...
func should_continue_spawning() -> bool:
	return (enemies_spawned < total_enemies) and (current_enemies_alive < max_enemies_alive)
...
func _generate_random_spawn_position(camera_pos: Vector2, screen_size: Vector2) -> Vector2:
	...
	var distance = spawn_distance + randf() * 100  # Some variation in distance
...
func _is_position_in_bounds(pos: Vector2) -> bool:
	...
	return (pos.x >= min_bounds.x and pos.x <= max_bounds.x and
			pos.y >= min_bounds.y and pos.y <= max_bounds.y)
